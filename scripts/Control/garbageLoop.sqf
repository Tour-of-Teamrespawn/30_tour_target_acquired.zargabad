	...
	{
		{
			...
			if (!isNull _unit) then
			{
				...
				{
					if (((_time + 300) < time)or(count TOUR_garbagearray > 50)) then
					{
						hideBody _unit;
						sleep 5;
						deleteVehicle _unit;
				...
				{
					if (((_time + 300 < time)or(count TOUR_garbagearray > 50))&&({_x distance _unit < 500} count (playableunits + switchableUnits) == 0)) then
					{
						deleteVehicle _unit;
					};
				};
